    ...
    try {
      ...
      if (video_url) {
        const duplicateCheck = await pool.query(
          'SELECT id FROM videos WHERE user_id = $1 AND url = $2',
      ...
      // Insert the video
      const result = await pool.query(`
        INSERT INTO videos (user_id, title, description, thumbnail_url, platform_id, video_id, url, duration, age_rating)
